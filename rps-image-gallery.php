...
class RPS_Image_Gallery {
	...
	public function cb_gallery_shortcode( $atts, $content = null ) {
		...
		$ids = $this->process_gallery_id( $id );
		...
		/**
		 * If there are not any attachments then bail.
		 */
		if ( empty( $attachments ) ) return '';

		/**
		 * Reorder the attachments if more than one gallery id is being requested.
		 */
		if ( count( $ids ) > 1 ) $attachments = $this->reorder_merged_attachments( $attachments, $orderby, $order );
